...
public class HangManUI extends Pane {
			...
			public void handle(ActionEvent event) {
				...
				eventHandler
						.respondToSwitchScreenRequest(HangManUIState.PLAY_GAME_STATE);
                                //Start new game automatically when clicked
	...
	private void initGameScreen() {
		...
		newGameButton = new Button();
                ...
                String newGameName = props.getProperty(HangManPropertyType.NEW_GAME_IMG_NAME);
                Image newGameImage = loadImage(newGameName);
                ImageView newGameView = new ImageView(newGameImage);
                Label NewGameLabel = new Label();
                newGameButton.setGraphic(newGameView);
    ...
    private void initStatsPane()
    {
        ...
        JScrollPane statsScroll = new JScrollPane(statsPane);
        statsScroll.setPreferredSize(new Dimension(700, 500));
        ...
        swingNode.setContent(statsScroll);
    ...
    /**
     * Goes back to splash screen when home button in help screen is clicked
     */
    public void goHome()
    {
        mainPane.getChildren().clear();
    ...
    /**
     * Gets the buttons that are implemented for guessing word
     * @return the letter buttons used for guessing the word
     */
    public HashMap<Character,Button> getButtons()
    {
