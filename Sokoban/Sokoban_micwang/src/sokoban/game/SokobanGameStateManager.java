...
public class SokobanGameStateManager {
    ...
    public Label timeManager()
    {
        final Timeline timeline = new Timeline(new KeyFrame(Duration.seconds(1), new EventHandler<ActionEvent>() {  
            @Override  
            public void handle(ActionEvent event) {  
                GregorianCalendar now = new GregorianCalendar();
                long timeInMillis = now.getTimeInMillis() - gameInProgress.getStartTime().getTimeInMillis();
                long hours = timeInMillis / (1000 * 3600);
                timeInMillis -= hours * (1000 * 3600);
                ...
                String time = "Time: " + hours + ":" + minutesText + ":" + secondsText;
                
                if(currentGameState == SokobanGameState.GAME_IN_PROGRESS)
                    clock.setText(time);
                }  
        }));  
